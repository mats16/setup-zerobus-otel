...
任意の OTLP/HTTP backend に送る最小フローです。CLI で確認するのは次の 2 項目だけです。

- OTLP の base URL (例: `https://otel.example.com`)。各シグナルのエンドポイントは `<base>/v1/logs`、`<base>/v1/metrics`、`<base>/v1/traces` になります。
- 静的な authorization token (`Authorization: Bearer <token>` として送信されます)

Databricks 固有のヘッダーやテーブル、MLflow Experiment などは作成されません。
...
- OTLP/HTTP エンドポイントと、その認証用 bearer token
